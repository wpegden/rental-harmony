...
theorem exists_terminal_of_boundaryOnlyUniqueCarrierCounterexampleData_of_exists_terminal_of_verticalNeighbor
    (hdata : TopDimNoOpenCrossingBoundaryOnlyUniqueCarrierCounterexampleData
      (T := T) (φ := φ))
    (hdown :
      ∀ μ : Section5PositiveNode (chosenMilestoneChain (φ := φ)) φ,
...
theorem
  ...
  have hμnotterm :
      ¬ IsTerminal (T := T) (chosenMilestoneChain (φ := φ)) φ (.positive μ) := by
    intro hterm
    have hνlevel : hdata.ν.level.1 + 1 = dimension := by
      simpa [hdata.ν.face_dim] using hdata.hνdim
    have hμdim_lt : μ.face.dim < dimension := by
      have hμdim : μ.face.dim = μ.level.1 + 1 := μ.face_dim
      rcases hμ with ⟨hlevel, -, -⟩
      omega
    exact (Nat.ne_of_lt hμdim_lt) hterm.1
  have hμnext :
      μ.face.ImageContainsMilestone (T := T)
        (chosenMilestoneChain (φ := φ)) φ.vertexMap μ.level.succ :=
    hμ.2.2
  rcases
      haway.two_doors_of_nextMilestone_awayFromBoundary μ
        (chosenMilestoneChain_nextMilestoneAwayFromBoundary_of_nonterminal
          (T := T) (φ := φ) hμnext hμnotterm)
        hμnotterm with
    ⟨a, b, hab, ha, hb, hall⟩
  have hνadj :
      Adj (T := T) (chosenMilestoneChain (φ := φ)) φ (.positive μ) (.positive hdata.ν) := by
    exact Or.inr <| Or.inl hμ
  rcases hall (.positive hdata.ν) hνadj with hνa | hνb
  · refine ⟨b, ⟨?_, hb⟩, ?_⟩
    · intro hbν
      apply hab
      calc
        a = .positive hdata.ν := hνa.symm
        _ = b := hbν.symm
    · intro z hz
      rcases hz with ⟨hzν, hzad⟩
      rcases hall z hzad with hza | hzb
      · exfalso
        exact hzν (hza.trans hνa.symm)
      · exact hzb
  · refine ⟨a, ⟨?_, ha⟩, ?_⟩
    · intro haν
      apply hab
      calc
        a = .positive hdata.ν := haν
        _ = b := hνb
    · intro z hz
      rcases hz with ⟨hzν, hzad⟩
      rcases hall z hzad with hza | hzb
...
theorem
    ...
    (hcont :
      ∀ μ : Section5PositiveNode (chosenMilestoneChain (φ := φ)) φ,
        ...
        ∀ w : Section5GraphNode (chosenMilestoneChain (φ := φ)) φ,
          w ≠ .positive hdata.ν →
          Adj (T := T) (chosenMilestoneChain (φ := φ)) φ (.positive μ) w →
          ∃ v : Section5GraphNode (chosenMilestoneChain (φ := φ)) φ,
            v ≠ .start ∧
              IsTerminal (T := T) (chosenMilestoneChain (φ := φ)) φ v) :
    ∃ v : Section5GraphNode (chosenMilestoneChain (φ := φ)) φ,
...
def boundaryOnlyUniqueCarrierDeletedSpurSubgraph
    (hdata : TopDimNoOpenCrossingBoundaryOnlyUniqueCarrierCounterexampleData
      (T := T) (φ := φ))
    (μ : Section5PositiveNode (chosenMilestoneChain (φ := φ)) φ) :
    (graph (T := T) (chosenMilestoneChain (φ := φ)) φ).Subgraph where
  verts := boundaryOnlyUniqueCarrierDeletedSpurSupport (T := T) (φ := φ) hdata μ
  Adj v w :=
    Adj (T := T) (chosenMilestoneChain (φ := φ)) φ v w ∧
      v ∈ boundaryOnlyUniqueCarrierDeletedSpurSupport (T := T) (φ := φ) hdata μ ∧
      w ∈ boundaryOnlyUniqueCarrierDeletedSpurSupport (T := T) (φ := φ) hdata μ
  adj_sub := fun h => h.1
  edge_vert := fun h => h.2.1
  symm := by
    intro v w h
    exact
      ⟨adj_symm (T := T) (c := chosenMilestoneChain (φ := φ)) (φ := φ) h.1, h.2.2, h.2.1⟩

@[simp] theorem boundaryOnlyUniqueCarrierDeletedSpurSubgraph_verts
    (hdata : TopDimNoOpenCrossingBoundaryOnlyUniqueCarrierCounterexampleData
      (T := T) (φ := φ))
    (μ : Section5PositiveNode (chosenMilestoneChain (φ := φ)) φ)
    (w : Section5GraphNode (chosenMilestoneChain (φ := φ)) φ) :
    w ∈ (boundaryOnlyUniqueCarrierDeletedSpurSubgraph (T := T) (φ := φ) hdata μ).verts ↔
      w ∈ boundaryOnlyUniqueCarrierDeletedSpurSupport (T := T) (φ := φ) hdata μ := by
  rfl

@[simp] theorem boundaryOnlyUniqueCarrierDeletedSpurSubgraph_adj_iff
    (hdata : TopDimNoOpenCrossingBoundaryOnlyUniqueCarrierCounterexampleData
      (T := T) (φ := φ))
    (μ : Section5PositiveNode (chosenMilestoneChain (φ := φ)) φ)
    {v w : Section5GraphNode (chosenMilestoneChain (φ := φ)) φ} :
    (boundaryOnlyUniqueCarrierDeletedSpurSubgraph (T := T) (φ := φ) hdata μ).Adj v w ↔
      Adj (T := T) (chosenMilestoneChain (φ := φ)) φ v w ∧
        v ∈ boundaryOnlyUniqueCarrierDeletedSpurSupport (T := T) (φ := φ) hdata μ ∧
        w ∈ boundaryOnlyUniqueCarrierDeletedSpurSupport (T := T) (φ := φ) hdata μ := by
  rfl

theorem twoNeighborSpec_of_not_terminal_belowTopDim_positive_of_alternativeSpecs
    (hzero : ChosenMilestoneChainLevelZeroBoundarySpec (T := T) (φ := φ))
    (hopen : ChosenMilestoneChainOpenCrossingSpec (T := T) (φ := φ))
    (halt :
      ChosenMilestoneChainPositiveLevelNoOpenCrossingAlternativeSpec
    ...
    ∃ a b : Section5GraphNode (chosenMilestoneChain (φ := φ)) φ,
      a ≠ b ∧
      Adj (T := T) (chosenMilestoneChain (φ := φ)) φ (.positive ξ) a ∧
      Adj (T := T) (chosenMilestoneChain (φ := φ)) φ (.positive ξ) b ∧
      ∀ w : Section5GraphNode (chosenMilestoneChain (φ := φ)) φ,
        Adj (T := T) (chosenMilestoneChain (φ := φ)) φ (.positive ξ) w → w = a ∨ w = b := by
  by_cases hnext :
      ξ.face.ImageContainsMilestone (T := T)
        (chosenMilestoneChain (φ := φ)) φ.vertexMap ξ.level.succ
  · exact
...
theorem
    ...
    (hξne : ξ ≠ hdata.ν)
    (hξadj : Adj (T := T) (chosenMilestoneChain (φ := φ)) φ (.positive μ) (.positive ξ)) :
    ¬ Adj (T := T) (chosenMilestoneChain (φ := φ)) φ (.positive ξ) (.positive hdata.ν) := by
  have hξsupport :=
  ...
  have hlt :=
    level_lt_of_positiveContinuationNeighbor_of_verticalAdj_boundaryOnlyUniqueCarrierCounterexampleData
      (T := T) (φ := φ) hdata hμ hξne hξadj
  intro hξν
  rcases hξν with hhor | hupper | hlower
  · rcases hhor with ⟨hlevel, -, -⟩
    have hEq : ξ.level.1 = hdata.ν.level.1 := by
  ...
  · rcases
      ...
      ⟨u, hu, huniq⟩
    have hEq : ξ = μ := by
      exact huniq ξ hupper |>.trans (huniq μ hμ).symm
    exact hξsupport.2 (by simpa [hEq])
  · rcases hlower with ⟨hlevel, -, -⟩
    omega

lemma graphNeighbor_mem_boundaryOnlyUniqueCarrierDeletedSpurSupport_of_mem_support_of_not_boundary
    (hdata : TopDimNoOpenCrossingBoundaryOnlyUniqueCarrierCounterexampleData
      (T := T) (φ := φ))
    {μ : Section5PositiveNode (chosenMilestoneChain (φ := φ)) φ}
    {w z : Section5GraphNode (chosenMilestoneChain (φ := φ)) φ}
    (hboundary :
      ¬ IsBoundaryOnlyUniqueCarrierDeletedSpurBoundary (T := T) (φ := φ) hdata μ w)
    (hadj : Adj (T := T) (chosenMilestoneChain (φ := φ)) φ w z) :
    z ∈ boundaryOnlyUniqueCarrierDeletedSpurSupport (T := T) (φ := φ) hdata μ := by
  refine ⟨?_, ?_⟩
  · intro hzν
    apply hboundary
    left
    simpa [hzν] using hadj
  · intro hzμ
    apply hboundary
    right
    simpa [hzμ] using hadj

theorem neighborSet_boundaryOnlyUniqueCarrierDeletedSpurSubgraph_eq_of_mem_support_of_not_boundary
    (hdata : TopDimNoOpenCrossingBoundaryOnlyUniqueCarrierCounterexampleData
      (T := T) (φ := φ))
    {μ : Section5PositiveNode (chosenMilestoneChain (φ := φ)) φ}
    {w : Section5GraphNode (chosenMilestoneChain (φ := φ)) φ}
    ...
    (hboundary :
      ¬ IsBoundaryOnlyUniqueCarrierDeletedSpurBoundary (T := T) (φ := φ) hdata μ w) :
    (boundaryOnlyUniqueCarrierDeletedSpurSubgraph (T := T) (φ := φ) hdata μ).neighborSet w =
      (graph (T := T) (chosenMilestoneChain (φ := φ)) φ).neighborSet w := by
  ext z
  constructor
  · intro hz
    exact hz.1
  · intro hz
    exact
      ⟨hz, hw,
        graphNeighbor_mem_boundaryOnlyUniqueCarrierDeletedSpurSupport_of_mem_support_of_not_boundary
          (T := T) (φ := φ) hdata hboundary hz⟩
...
theorem
    ...
    (hξadj : Adj (T := T) (chosenMilestoneChain (φ := φ)) φ (.positive μ) (.positive ξ)) :
    ∃! w : Section5GraphNode (chosenMilestoneChain (φ := φ)) φ,
      (boundaryOnlyUniqueCarrierDeletedSpurSubgraph (T := T) (φ := φ) hdata μ).Adj
        (.positive ξ) w := by
  classical
  let H := boundaryOnlyUniqueCarrierDeletedSpurSubgraph (T := T) (φ := φ) hdata μ
  have hξsupport :
      .positive ξ ∈ boundaryOnlyUniqueCarrierDeletedSpurSupport (T := T) (φ := φ) hdata μ :=
    positiveContinuationNeighbor_mem_boundaryOnlyUniqueCarrierDeletedSpurSupport
      (T := T) (φ := φ) hdata hξne hξadj
  have hξterm :
      ¬ IsTerminal (T := T) (chosenMilestoneChain (φ := φ)) φ (.positive ξ) :=
  ...
  rcases
    ...
    ⟨a, b, hab, ha, hb, hall⟩
  have hξμ :
      Adj (T := T) (chosenMilestoneChain (φ := φ)) φ (.positive ξ) (.positive μ) := by
    exact adj_symm (T := T) (c := chosenMilestoneChain (φ := φ)) (φ := φ) hξadj
  have hξν :
      ¬ Adj (T := T) (chosenMilestoneChain (φ := φ)) φ (.positive ξ) (.positive hdata.ν) :=
    not_adj_counterexampleNode_of_positiveContinuationNeighbor_of_verticalAdj_boundaryOnlyUniqueCarrierCounterexampleData
      (T := T) (φ := φ) hdata hμ hξne hξadj
  rcases hall (.positive μ) hξμ with hμa | hμb
  · refine ⟨b, ?_, ?_⟩
    · refine ⟨hb, hξsupport, ?_⟩
      refine ⟨?_, ?_⟩
      · intro hbν
        exact hξν (by simpa [hbν] using hb)
      · intro hbμ
        exact hab (hμa.symm.trans hbμ.symm)
    · intro z hz
      have hzg : Adj (T := T) (chosenMilestoneChain (φ := φ)) φ (.positive ξ) z := hz.1
      rcases hall z hzg with hza | hzb
      · exfalso
        exact hz.2.2.2 (by simpa [hμa] using hza)
      · exact hzb
  · refine ⟨a, ?_, ?_⟩
    · refine ⟨ha, hξsupport, ?_⟩
      refine ⟨?_, ?_⟩
      · intro haν
        exact hξν (by simpa [haν] using ha)
      · intro haμ
        exact hab (haμ.trans hμb)
    · intro z hz
      have hzg : Adj (T := T) (chosenMilestoneChain (φ := φ)) φ (.positive ξ) z := hz.1
      rcases hall z hzg with hza | hzb
      · exact hza
...
-/
structure ChosenMilestoneChainBelowTopDimPositiveLevelDescentSpec where
  exists_lowerLevel_positive_of_not_terminal_belowTopDim :
    ∀ ξ : Section5PositiveNode (chosenMilestoneChain (φ := φ)) φ,
      ξ.face.dim < dimension →
      ¬ IsTerminal (T := T) (chosenMilestoneChain (φ := φ)) φ (.positive ξ) →
      ∃ ζ : Section5PositiveNode (chosenMilestoneChain (φ := φ)) φ,
...
/--
Stronger local continuation principle sufficient for the direct below-top-dimensional route.

Since the goal is only existence of some terminal node in the whole chosen-chain graph, a
strictly level-lowering positive continuation from every below-top-dimensional nonterminal node is
enough: one can recurse on the level until a terminal witness appears.
-/
...
def chosenMilestoneChainBelowTopDimPositiveTerminalSpec_of_levelDescent
    (hdesc :
  ...
  have hP : ∀ k, P k := by
    ...
    intro k ih ξ hξlevel hξdim
    by_cases hξterm :
        IsTerminal (T := T) (chosenMilestoneChain (φ := φ)) φ (.positive ξ)
    · exact ⟨.positive ξ, by simp, hξterm⟩
    · rcases
          hdesc.exists_lowerLevel_positive_of_not_terminal_belowTopDim ξ hξdim hξterm with
        ⟨ζ, _, hζlt⟩
      have hζlt' : ζ.level.1 < k := by
        simpa [hξlevel] using hζlt
      have hζdim : ζ.face.dim < dimension := by
        calc
          ζ.face.dim = ζ.level.1 + 1 := ζ.face_dim
          _ < ξ.level.1 + 1 := Nat.succ_lt_succ hζlt
          _ = ξ.face.dim := by rw [ξ.face_dim]
          _ < dimension := hξdim
      exact ih ζ.level.1 hζlt' ζ rfl hζdim
  exact hP ξ.level.1 ξ rfl hξdim

/--
More realistic direct-route reduction: level `0` is handled separately, and strict descent is
only required at positive levels.

This matches the actual shape of the Section 5 graph better, since a level-`0` positive node
cannot possibly have a lower positive neighbor.
-/
theorem
  ...
  exists_terminal_of_levelZero_belowTopDim :
    ∀ ξ : Section5PositiveNode (chosenMilestoneChain (φ := φ)) φ,
      ...
      ∃ v : Section5GraphNode (chosenMilestoneChain (φ := φ)) φ,
        v ≠ .start ∧
          IsTerminal (T := T) (chosenMilestoneChain (φ := φ)) φ v
  exists_lowerLevel_positive_of_nextMilestone_not_terminal_belowTopDim :
    ∀ ξ : Section5PositiveNode (chosenMilestoneChain (φ := φ)) φ,
      0 < ξ.level.1 →
      ξ.face.dim < dimension →
      ξ.face.ImageContainsMilestone (T := T)
      ...
      ∃ ζ : Section5PositiveNode (chosenMilestoneChain (φ := φ)) φ,
        Adj (T := T) (chosenMilestoneChain (φ := φ)) φ (.positive ξ) (.positive ζ) ∧
          ζ.level.1 < ξ.level.1
  exists_lowerLevel_positive_of_openCrossing_not_terminal_belowTopDim :
    ∀ ξ : Section5PositiveNode (chosenMilestoneChain (φ := φ)) φ,
      0 < ξ.level.1 →
      ξ.face.dim < dimension →
      ¬ ξ.face.ImageContainsMilestone (T := T)
...
def
    ...
    (hcase :
      ChosenMilestoneChainBelowTopDimPositiveBaseCaseAndCaseSplitDescentSpec
        (T := T) (φ := φ)) :
    ChosenMilestoneChainBelowTopDimPositiveBaseCaseAndLevelDescentSpec
      (T := T) (φ := φ) := by
  refine
    { exists_terminal_of_levelZero_belowTopDim :=
        hcase.exists_terminal_of_levelZero_belowTopDim
      exists_lowerLevel_positive_of_not_terminal_belowTopDim := ?_ }
  intro ξ hk hξdim hξterm
  by_cases hupper :
      ξ.face.ImageContainsMilestone (T := T)
        (chosenMilestoneChain (φ := φ)) φ.vertexMap ξ.level.succ
  · exact
      hcase.exists_lowerLevel_positive_of_nextMilestone_not_terminal_belowTopDim
        ξ hk hξdim hupper hξterm
  · by_cases hopen :
      ξ.face.ImageMeetsOpenMilestoneSegment (T := T)
        (chosenMilestoneChain (φ := φ)) φ.vertexMap ξ.level
    · exact
        hcase.exists_lowerLevel_positive_of_openCrossing_not_terminal_belowTopDim
          ξ hk hξdim hupper hopen hξterm
    · exact
        exists_lowerLevel_positive_of_missingNextMilestone_positiveLevel_of_not_openCrossing_of_largeLowerPrefixCarrierSpec
          (T := T) (φ := φ) hlarge hk hupper hopen

def chosenMilestoneChainBelowTopDimPositiveTerminalSpec_of_baseCaseAndLevelDescent
    (hdesc :
      ChosenMilestoneChainBelowTopDimPositiveBaseCaseAndLevelDescentSpec
        (T := T) (φ := φ)) :
    ChosenMilestoneChainBelowTopDimPositiveTerminalSpec
  ...
  have hP : ∀ k, P k := by
    ...
    · by_cases hk : 0 < ξ.level.1
      · rcases
            hdesc.exists_lowerLevel_positive_of_not_terminal_belowTopDim ξ hk hξdim hξterm with
          ⟨ζ, _, hζlt⟩
        have hζlt' : ζ.level.1 < k := by
          simpa [hξlevel] using hζlt
        have hζdim : ζ.face.dim < dimension := by
        ...
        exact ih ζ.level.1 hζlt' ζ rfl hζdim
      · exact
          hdesc.exists_terminal_of_levelZero_belowTopDim ξ
            (Nat.eq_zero_of_not_pos hk) hξdim
  exact hP ξ.level.1 ξ rfl hξdim
...
def
    chosenMilestoneChainBoundaryOnlyUniqueCarrierBypassSpec_of_belowTopDimPositiveLevelDescentSpec_of_two_lt_dimension
    (haway : ChosenMilestoneChainNextMilestoneAwayFromBoundarySpec (T := T) (φ := φ))
    (hdim : 2 < dimension)
    (hdesc :
      ChosenMilestoneChainBelowTopDimPositiveLevelDescentSpec
  ...
  chosenMilestoneChainBoundaryOnlyUniqueCarrierBypassSpec_of_belowTopDimPositiveTerminalSpec_of_two_lt_dimension
    (T := T) (φ := φ) haway hdim
    (chosenMilestoneChainBelowTopDimPositiveTerminalSpec_of_levelDescent
      (T := T) (φ := φ) hdesc)

def
    chosenMilestoneChainBoundaryOnlyUniqueCarrierBypassSpec_of_belowTopDimPositiveBaseCaseAndLevelDescentSpec_of_two_lt_dimension
    (haway : ChosenMilestoneChainNextMilestoneAwayFromBoundarySpec (T := T) (φ := φ))
    (hdim : 2 < dimension)
    (hdesc :
      ChosenMilestoneChainBelowTopDimPositiveBaseCaseAndLevelDescentSpec
...
def chosenMilestoneChainBoundaryOnlyUniqueCarrierBypassSpec_of_continuationNeighborTerminal
  ...
  exact
    exists_terminal_of_boundaryOnlyUniqueCarrierCounterexampleData_of_exists_terminal_of_continuationNeighbor
      (T := T) (φ := φ) haway hdata (hcont hdata)

def chosenMilestoneChainBoundaryOnlyUniqueCarrierBypassSpec_of_verticalNeighborTerminal
    (hdown :
      ∀ hdata : TopDimNoOpenCrossingBoundaryOnlyUniqueCarrierCounterexampleData
        (T := T) (φ := φ),
        ∀ μ : Section5PositiveNode (chosenMilestoneChain (φ := φ)) φ,
  ...
  intro hdata
  exact
    exists_terminal_of_boundaryOnlyUniqueCarrierCounterexampleData_of_exists_terminal_of_verticalNeighbor
      (T := T) (φ := φ) hdata (hdown hdata)
